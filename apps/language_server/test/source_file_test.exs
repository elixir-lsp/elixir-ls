defmodule ElixirLS.LanguageServer.SourceFileTest do
  ...
  describe "apply_content_changes" do
    ...
    test "empty update" do
      assert %SourceFile{text: "abc123", version: 0} = SourceFile.apply_content_changes(new("abc123"), [])
    end

    test "full update" do
      assert %SourceFile{text: "efg456", version: 1} = SourceFile.apply_content_changes(new("abc123"), [%{"text" => "efg456"}])

      assert %SourceFile{text: "world", version: 2} = SourceFile.apply_content_changes(new("abc123"), [
        %{"text" => "hello"},
        %{"text" => "world"}
        ])
    end
    ...
    test "incrementally removing content" do
      ...
      assert %SourceFile{text: "function abc() {\n  console.log(\"\");\n}", version: 1} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => "",
    ...
    test "incrementally replacing multi-line content, less lines" do
      sf = new("a1\nb1\na2\nb2\na3\nb3\na4\nb4\n")
      assert %SourceFile{text: "a1\nb1\na2\nb2xx\nyy", version: 1} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => "xx\nyy",
    ...
    test "incrementally replacing multi-line content, huge number of lines" do
      ...
      assert %SourceFile{text: res, version: 1} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => text,
          "range" => range_for_substring(sf, "\ncc")
        }
    ...
    test "several incremental content changes" do
      sf = new("function abc() {\n  console.log(\"hello, world!\");\n}")
      assert %SourceFile{text: "function abcdefghij() {\n  console.log(\"hello, test case!!!\");\n}", version: 3} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => "defg",
    ...
    test "multi-character replace" do
      sf = new("foo\nbar")
      assert %SourceFile{text: "foo\nfoobar", version: 1} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => "foobar",
          "range" => range_create(1, 0, 1, 3)
        }
    ...
    test "invalid update range - after the document ends -> after the document ends" do
      sf = new("foo\nbar")
      assert %SourceFile{text: "foo\nbarabc123", version: 1} = SourceFile.apply_content_changes(sf, [
        %{
          "text" => "abc123",
    ...
    test "invalid update range - before the document starts -> after the document ends" do
      sf = new("foo\nbar")
      assert %SourceFile{text: "entirely new content", version: 1} = SourceFile.apply_content_changes(sf, [
        %{
