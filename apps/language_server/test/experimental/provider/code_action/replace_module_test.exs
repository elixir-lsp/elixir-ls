defmodule ElixirLS.LanguageServer.Experimental.Provider.CodeAction.ReplaceModuleTest do
  ...
  def struct_diagnostic_message(module) do
    """
    (CompileError) #{module}.__struct__/1 is undefined, cannot expand struct #{module}. Make sure the struct name is correct. If the struct name exists and is correct but it still cannot be found, you likely have cyclic momdule usage in your code
  ...
  test "replace module in the function call" do
    actual_code = ~S[
      defmodule MyModule do
        def foo do
          ExampleDefaultArgs.my_func("text")
  ...
  test "replace unknown struct" do
    actual_code = ~S[
      defmodule MyModule do
        def foo do
          %ExampleStruct{}
