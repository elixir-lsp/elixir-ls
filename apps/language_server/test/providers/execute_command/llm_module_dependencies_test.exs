defmodule ElixirLS.LanguageServer.Providers.ExecuteCommand.LlmModuleDependenciesTest do
  ...
  if Version.match?(System.version(), ">= 1.18.0") do
    ...
    test "handles remote call with arity nil (function name only)" do
      ...
      assert {:ok, result} =
               LlmModuleDependencies.execute(["ElixirLS.Test.ModuleDepsC.function_in_c"], state)
    ...
    test "filters transitive dependencies by function for remote calls" do
      ...
      assert {:ok, result} =
               LlmModuleDependencies.execute(
                 ["ElixirLS.Test.ModuleDepsA.function_with_direct_call/0"],
                 state
               )
      ...
      assert {:ok, result2} =
               LlmModuleDependencies.execute(
                 ["ElixirLS.Test.ModuleDepsA.multiple_dependencies/0"],
                 state
               )
      ...
      assert {:ok, unfiltered_result} =
               LlmModuleDependencies.execute(["ElixirLS.Test.ModuleDepsA"], state)
    ...
    test "filters reverse transitive dependencies by function for remote calls" do
      ...
      assert {:ok, unfiltered_result} =
               LlmModuleDependencies.execute(["ElixirLS.Test.ModuleDepsC"], state)
