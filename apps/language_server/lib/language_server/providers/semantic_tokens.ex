defmodule ElixirLS.LanguageServer.Providers.SemanticTokens do
  ...
  alias ElixirLS.LanguageServer.{SourceFile, CodeUnit}
  ...
  defp encode(spans, lines) do
    ...
    {rows, _prev_line, _prev_start} =
      Enum.reduce(spans, {[], 0, 0}, fn {sl, sc, _el, ec, type, mods}, {acc, prev_line, prev_start} ->
        line0 = sl - 1

        with tindex when is_integer(tindex) <- Map.get(@type_index, type),
             true <- line0 >= 0 and line0 < line_count do
          line = elem(lines, line0)
          start16 = CodeUnit.utf16_offset(line, sc - 1)
          end16 = CodeUnit.utf16_offset(line, ec - 1)
          ...
          if length > 0 do
            ...
            {[row | acc], line0, start16}
          else
            {acc, prev_line, prev_start}
          end
        else
          _ -> {acc, prev_line, prev_start}
        end
      end)
