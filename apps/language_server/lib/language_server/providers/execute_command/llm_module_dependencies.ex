defmodule ElixirLS.LanguageServer.Providers.ExecuteCommand.LlmModuleDependencies do
  ...
  defp get_module_dependencies_filtered_by_function(module, function, arity) do
    ...
    transitive_deps = get_transitive_dependencies_from_direct(module, filtered_direct_deps, :compile)
    reverse_transitive_deps = get_reverse_transitive_dependencies_from_direct(module, filtered_reverse_deps, :compile)
  ...
  defp get_direct_dependencies_filtered_by_function(module, function, arity) do
    ...
    deps = Enum.reduce(calls, %{
      ...
      matching_call_infos = Enum.filter(call_infos, fn info ->
        info.caller_module == module and
        matches_function_call?(info.caller_function, function, arity)
      end)
      ...
      Enum.reduce(matching_call_infos, acc, fn info, inner_acc ->
        ...
        case info.kind do
          kind when kind in [:imported_function, :imported_macro] ->
            %{inner_acc | imports: MapSet.put(inner_acc.imports, {callee_module, name, call_arity})}
          ...
          kind when kind in [:remote_function, :remote_macro] ->
            %{inner_acc | function_calls: MapSet.put(inner_acc.function_calls, {callee_module, name, call_arity})}
  ...
  defp get_reverse_dependencies_filtered_by_function(module, function, arity) do
    ...
    deps = Enum.reduce(calls, %{
      ...
      info, inner_acc ->
        ...
        case info.kind do
          kind when kind in [:imported_function, :imported_macro] ->
            %{inner_acc | imports: MapSet.put(inner_acc.imports, %{function: {callee_module, name, call_arity}, importing_module: info.caller_module})}
          ...
          kind when kind in [:remote_function, :remote_macro] ->
            %{inner_acc | function_calls: MapSet.put(inner_acc.function_calls, %{function: {callee_module, name, call_arity}, caller_module: info.caller_module})}
  ...
  defp matches_function_call?(caller_function, target_function, _target_arity) when is_atom(caller_function) do
