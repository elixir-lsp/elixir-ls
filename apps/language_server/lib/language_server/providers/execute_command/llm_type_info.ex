defmodule ElixirLS.LanguageServer.Providers.ExecuteCommand.LlmTypeInfo do
  ...
  defp format_callback({{name, arity}, specs}) do
    ...
    kind =
      if String.starts_with?(to_string(name), "MACRO-") do
        :macrocallback
      else
        :callback
      end
  ...
  defp filter_type_info_by_function(type_info, function, arity) do
    ...
    filtered_callbacks =
      Enum.filter(type_info.callbacks, fn callback -> match_function.(callback.name) end)

    filtered_dialyzer_contracts =
      Enum.filter(type_info.dialyzer_contracts, fn contract -> match_function.(contract.name) end)
