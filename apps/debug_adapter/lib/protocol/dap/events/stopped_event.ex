...
defmodule GenDAP.Events.StoppedEvent do
  ...
  * type: Message type.
  ...
  typedstruct do
    ...
    field :body, %{required(:reason) => String.t(), optional(:description) => String.t(), optional(:text) => String.t(), optional(:thread_id) => integer(), optional(:preserve_focus_hint) => boolean(), optional(:all_threads_stopped) => boolean(), optional(:hit_breakpoint_ids) => list(integer())}, enforce: true
  end
  ...
  def schematic() do
    schema(__MODULE__, %{
      ...
      :event => "stopped",
      :body => map(%{
        {"reason", :reason} => oneof(["step", "breakpoint", "exception", "pause", "entry", "goto", "function breakpoint", "data breakpoint", "instruction breakpoint", str()]),
